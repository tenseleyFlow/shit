...
# SMOKE_NAME: git-branch-D-undo-linux
...
if [ "$(uname -s)" != "Linux" ]; then
    smoke_log "SKIP: git-branch-D-undo-linux is Linux-only"
    exit 0
fi
if [ -z "${GIT_BIN}" ]; then
...
smoke_log "PASS: git-branch-D-undo-linux (unpacked branch restored, ${NUM_PRE} pre-images)"
