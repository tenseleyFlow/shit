...
# SMOKE_NAME: git-checkout-branch-undo-fbsd
...
if [ "$(uname -s)" != "FreeBSD" ]; then
    smoke_log "SKIP: git-checkout-branch-undo-fbsd is FreeBSD-only"
    exit 0
fi
if [ -z "${GIT_BIN}" ]; then
...
MAIN_FILE_SHA="$(/sbin/sha256 -q "${REPO}/file.txt")"
...
POST_FILE_SHA="$(/sbin/sha256 -q "${REPO}/file.txt")"
...
smoke_log "PASS: git-checkout-branch-undo-fbsd (HEAD feat->main restored, working tree intact, ${NUM_PRE} pre-images)"
