...
# SMOKE_NAME: git-checkout-file-undo-fbsd
...
if [ "$(uname -s)" != "FreeBSD" ]; then
    smoke_log "SKIP: git-checkout-file-undo-fbsd is FreeBSD-only"
    exit 0
fi
if [ -z "${GIT_BIN}" ]; then
...
DIRTY_SHA="$(/sbin/sha256 -q "${REPO}/README.md")"
...
POST_UNDO_SHA="$(/sbin/sha256 -q "${REPO}/README.md")"
...
smoke_log "PASS: git-checkout-file-undo-fbsd (uncommitted edit restored, ${NUM_PRE} pre-images)"
