...
# SMOKE_NAME: make-install-undo-linux
...
# EXCLUDED_REASON: 
#
# AR05.1 smoke — Linux twin of make-install-undo-fbsd.sh. `make
# install` writes files into a prefix OUTSIDE the watched cwd
# subtree. `shit undo` should reverse those writes even though the
# kernel-capture tier can't see them (out-of-watch on the LSM /
# fanotify path).
#
# The capture mechanism for cross-watch observation is the
# LD_PRELOAD shim (S24.D + W06.A.*): the shim interposes libc-level
# mutations and notifies the daemon over `$XDG_RUNTIME_DIR/
# shit-shim.sock`. On the ubuntu-24.04 hosted runner the helper is
# unprivileged so fanotify is unavailable AND the LSM tier needs
# `lsm=...,bpf` in the kernel cmdline (not set on stock GHA) —
# the LD_PRELOAD shim is the load-bearing capture here.
#
# Outcomes (mirroring the FreeBSD smoke):
...
if [ "$(uname -s)" != "Linux" ]; then
    smoke_log "SKIP: make-install-undo-linux is Linux-only (uname=$(uname -s))"
    exit 0
fi
...
PRE_HELLO_SHA="$(sha256sum "${WATCHED}/hello" | awk '{print $1}')"
PRE_README_SHA="$(sha256sum "${WATCHED}/README" | awk '{print $1}')"
...
POST_HELLO_SHA="$(sha256sum "${WATCHED}/hello" | awk '{print $1}')"
POST_README_SHA="$(sha256sum "${WATCHED}/README" | awk '{print $1}')"
...
if [ "${HELLO_GONE}" = "yes" ] && [ "${README_GONE}" = "yes" ]; then
    ...
    smoke_log "PASS: make-install-undo-linux (Outcome A)"
    exit 0
fi
...
if [ "${UNDO_RC}" -ne 0 ] && grep -qE "install-target|out-of-scope|outside|refus|/bin/hello|/share/README" "${SHIT_SMOKE_TMP}/undo.log"; then
    ...
    smoke_log "PASS: make-install-undo-linux (Outcome B)"
    exit 0
fi

# Outcome C — silent partial undo. FAIL.
smoke_log "OUTCOME C — silent partial undo (LD_PRELOAD shim → journal wiring missing)"
