...
# SMOKE_NAME: git-reset-hard-undo-linux
...
# EXCLUDED_REASON: 
#
# G01.1 smoke — `git reset --hard <prior>` then `shit undo` returns
# the repo to its pre-reset state.
#
...
#
# All of these are pre-imaged by the LSM tier via either
# W06.A.4 (rename-with-pre-image atomic-replace, for .git/index's
# tmpfile+rename swap) or W09.5 (open-O_TRUNC FilePreImage capture,
# for the ref files and working-tree overwrite). The existing
# AR01.1 plumbing for git-commit covers the same surface.
#
...
if [ "$(uname -s)" != "Linux" ]; then
    smoke_log "SKIP: git-reset-hard-undo-linux is Linux-only (uname=$(uname -s))"
    exit 0
fi
if [ -z "${GIT_BIN}" ]; then
...
FILE_C_SHA="$(sha256sum "${REPO}/README.md" | cut -d' ' -f1)"
...
POST_UNDO_FILE_SHA="$(sha256sum "${REPO}/README.md" | cut -d' ' -f1)"
...
smoke_log "PASS: git-reset-hard-undo-linux (HEAD A->C restored, working tree intact, ${NUM_PREIMAGES} pre-images captured)"
